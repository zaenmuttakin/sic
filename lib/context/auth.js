    ...
    checkSession()
      ...
      .catch((error) => {
        ...
        logout().then(() => {
          localStorage.removeItem("user");
          const queryParams = new URLSearchParams({
            alert: JSON.stringify(error.message || "Session check failed"),
            type: JSON.stringify("error"),
          }).toString();
          router.push("/?" + queryParams);
        });
      });
    const interval = setInterval(checkSession, 30 * 60 * 1000); // Check session every 30 minutes
