const gas_id =
  "AKfycbyUZFAra-EjdRDb69Wx0fyiHmCISQdWNA09UUg7fLpTKvxEqWvxEl9jdw81kPiURei0";
...
export async function addValidate(params) {
  const formData = new FormData();
  formData.append("data", JSON.stringify(params));
  try {
    const res = await fetch(`${gas_url}?p=addValidate`, {
      method: "POST",
      body: formData,
    });
    return await res.json();
...
export async function getValidate(params) {
  ...
  formData.append("data", JSON.stringify(params));
  try {
    const res = await fetch(`${gas_url}?p=getValidate`, {
      method: "POST",
      body: formData,
    });
    return await res.json();
...
export async function checkBinList(params) {
  const formData = new FormData();
  formData.append("data", JSON.stringify(params));
  try {
    const res = await fetch(`${gas_url}?p=checkBinList`, {
      method: "POST",
      body: formData,
    });
    return await res.json();
